    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/usuario1/login").permitAll() // Permite acesso ao endpoint de login sem autenticação
                        .requestMatchers(HttpMethod.GET, "/auth").permitAll()// Permite acesso ao endpoint GET /auth sem autenticação
                        .requestMatchers(HttpMethod.POST, "/usuario1").permitAll() // Permite acesso ao endpoint POST /usuario sem autenticação
                        .requestMatchers("/usuario1/**").authenticated() // Requer autenticação para qualquer endpoint que comece com /usuario/
